...
\begin{table}[htbp]
    \caption{Keywords for SVV stabilisation (Navier-Stokes solver only).}
    \begin{tabular}{|l|p{10cm}|p{2.2cm}|}
    \hline
    \multicolumn{1}{|c|}{Keyword} & \multicolumn{1}{c|}{Description} & \multicolumn{1}{c|}{Default value} \\ \hline

    Enable SVV            & \textit{LOGICAL}: & .FALSE. \\ \hline
    SVV viscosity 1       & \textit{REAL/CHARACTER(*)}: Viscosity parameter~$\mu_1$ or "Smagorinsky" for the SVV-LES approach & 0.0 \\ \hline
    SVV alpha viscosity 1 & \textit{REAL}: Viscosity parameter~$\alpha_1$ & 0.0 \\ \hline
    SVV viscosity 2       & \textit{REAL}: Viscosity parameter~$\mu_2$, or~$C_s$ in the case of SVV-LES & $\mu_2=0.0$ \newline $C_s=0.2$ \\ \hline
    SVV alpha viscosity 2 & \textit{REAL}: Viscosity parameter~$\alpha_2$ & 0.0 \\ \hline
    ...
    SVV filter type       & \textit{CHARACTER(*)}: Options are:
        ...
        \end{itemize}
    & \textbf{Mandatory keyword} \\ \hline
    SVV dissipation type  & \textit{CHARACTER(*)}: Options are:
        \begin{itemize}
            \item Physical
            \item Guermond
        \end{itemize}
