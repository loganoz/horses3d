...
\textit{WARNING: The functionality explained in this section is still experimental and may change in future iterations.}
\textit{WARNING: This method does not support MPI parallelisation yet.}\newline
...
Shock sensor & \textit{CHARACTER}: Type of sensor to be used to detect discontinuous regions. Options are:
    \begin{itemize}
        ...
        \item Modal: based on the relative weight of the higher order modes (not working yet),
...
Shock first method & \textit{CHARACTER}: Method to be used in the middle region of the sensor ($a\in[0,1]$). Options are:
    \begin{itemize}
        ...
        \item SSFV: Apply a more dissipative version of the divergence operator.
...
Shock mu 1 & \textit{REAL/CHARACTER(*)}: Viscosity parameter~$\mu_1$ or "Smagorinsky" for the SVV-LES approach & 0.0 \\ \hline
Shock alpha 1 & \textit{REAL}: Viscosity parameter~$\alpha_1$ & 0.0 \\ \hline
Shock mu 2 & \textit{REAL}: Viscosity parameter~$\mu_2$, or~$C_s$ in the case of SVV-LES & $\mu_2=0.0$ \newline $C_s=0.2$ \\ \hline
Shock alpha 2 & \textit{REAL}: Viscosity parameter~$\alpha_2$ & 0.0 \\ \hline
Shock alpha/mu & & \\ \hline
SSFV blending 1 & & \\ \hline
SSFV blending 2 & & \\ \hline
...
SVV filter shape & \textit{CHARACTER(*)}: Options are:
    \begin{itemize}
        ...
        \item Sharp
        \item Exponential
    \end{itemize} & \textbf{Mandatory keyword} \\ \hline
SVV filter type & \textit{CHARACTER(*)}: Options are:
    \begin{itemize}
        \item Low-pass
        \item High-pass
    \end{itemize} & \textbf{Mandatory keyword} \\ \hline
Sensor variable & & \\ \hline
Sensor lower limit & & \\ \hline
Sensor higher limit & & \\ \hline
Sensor TE min N & & \\ \hline
Sensor TE delta N & & \\ \hline
Sensor TE derivative && \\ \hline
\end{longtable}
...
Wall Boundaries 				& \textit{CHARACTER(*)}: Array containing the name of each boundary to be used. In the form: '[bc1,bc2,bc3]'. Mandatory for using the wall function. & -- \\ \hline
