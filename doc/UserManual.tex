...
The code has an implementation of the Full Approximation Scheme (FAS) nonlinear $p$-multigrid method.
...
rk order & \textit{INTEGER}: Order of Runge-Kutta method optimized for steady-state solver. Possible orders are from 2 to 7. & 2 \\ \hline
...
The code has an implementation of the Full Approximation Scheme (FAS) nonlinear $p$-multigrid method.
The main keywords to use it are shown in Table \ref{tab:multigridKey}.
...
\textbf{simulation type} & \textit{CHARACTER}: Specifies if HORSES3D must perform a ’steady-state’ or a ’time-accurate’. If 'time-accurate' the solver switches to BDF integration (the exact method can be set using 'bdf order' option) and uses FAS as a pseudo problem solver. Compatible only with 'FAS'. & 'steady-state' \\ \hline
...
mg smoother     & \textit{CHARACTER}: The smoothing technique to be used. The keywords and possible smoothers are the same as the 'explicit method' in \ref{tab:explicitKey}. & RK3 \\ \hline
