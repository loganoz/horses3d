...
max newton iter    & \textit{INTEGER}: Maximum number of Newton iterationsfor BDF solver.  & 30 \\ \hline
...
type             & \textit{INTEGER}: Specifies the type of Jacobian matrix to be computed. Options are:\
				\begin{enumerate}
					\item Numerical Jacobian: Uses coloring algorithm for computing Jacobian (only available with shared memory parallelization).
					\item Analytical Jacobian: Available with shared (OpenMP) or distributed (MPI) memory parallelization for advective and/or diffusive nonlinear conservation laws. 
					\end{enumerate}
										& \textbf{Mandatory Keyword} \\ \hline
...
\end{longtable}
.TRUE.=Preallocate / .FALSE.=Perform allocation every time the Jacobian is constructed
...
\textbf{ATTENTION:} Real-time keywords may not work in parallel MPI computations. I depends on how the 
...
-{}-output-mode= 	& 
			\textit{CHARACTER}: Either \textit{multizone} or \textit{FE}. The option \textit{multizone} generates a Tecplot zone for each element. The option \textit{FE} generates only one zone for the fluid with finite element definitions (this format is faster to read by Paraview and Tecplot, but currently it is only supported for \textit{-{}-output-order=1}).  & 
