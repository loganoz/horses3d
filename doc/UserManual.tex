...
To visualize the mesh, the user needs to run the simulation first, which creates (*.bmesh) file. Then, simply:

\begin{lstlisting}[language=bash]
	$ horses2plt MeshFile.bmesh
\end{lstlisting}

If the mesh was created with Kopriva's SpecMesh, the better idea would be to use those tools to visalize the mesh right after it was created (see FIXME - ref to Kopriva guide).
