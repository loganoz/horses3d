...
!////////////////////////////////////////////////////////////////////////////////////////
!
MODULE foamCreateMeshFileConverter
    USE SMConstants
    USE Storage
...
!
            ...
            write(fid,'(I0)') foamMesh % nPointsUnique
            write(fid,'(A)')'('

!        Write the points
!        ----------------
			DO eID=1, foamMesh % nPoints
				ASSOCIATE(p =>foamMesh % horsesPoints (eID))
				if (p % master) then
					associate ( e => mesh % elements(p % eID) )
					write(fid,'(A,E18.10,E18.10,E18.10,A)')'( ', e % xOut(:,p%i,p%j,p%k), ' )'
					end associate
				end if 
				END ASSOCIATE
			END DO 
...
!
            ...
            write (fid,'(I0)')foamMesh % nFacesShared+foamMesh % nFacesUnshared+foamMesh % nFacesBoundaries - foamMesh % nMultipleFaces
            write(fid,'(A)')'('
            ...
            DO i=1,foamMesh % nFacesUnshared
				if (foamMesh % faceUnshared (i) % master) then
...
!
            INTEGER         :: i
            write (fid,'(I0)')foamMesh % nFacesShared + foamMesh % nMultipleFaces
            write(fid,'(A)')'('
            DO i=1,foamMesh % nFacesShared
			...
			DO i=1,foamMesh % nFacesUnshared
				if (foamMesh % faceUnshared (i) % master) then
					write(fid,'(I0)') foamMesh % faceUnshared (i)   % faceNeighbour 
				end if 
            END DO
            write(fid,'(A)')')'
...
!
            ...
            DO i=1,size(foamMesh % boundaries)
                ...
                write(fid,'(A,I0,A)') '        startFace       ', foamMesh % boundaries(i) % faceStart - foamMesh % nMultipleFaces,';'
                write(fid,'(A)') '    }'
            END DO
            write(fid,'(A)')')'
            write(fid,'(A)')''
...
END MODULE foamCreateMeshFileConverter
