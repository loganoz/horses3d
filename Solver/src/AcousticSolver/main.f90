...
!$omp end parallel do
   ...
   pure subroutine evalManufacturedSolution(x, y, z, t, res)
      ...
      call SetSolver(ACOUSTIC_SOLVER)
!
!     -----------------------------------------
...
!
      if ( MPI_Process % doMPIAction ) then
         CALL Main_Header("HORSES3D High-Order (DG) Spectral Element Parallel Acoustic Solver",__DATE__,__TIME__)

      else
         CALL Main_Header("HORSES3D High-Order (DG) Spectral Element Sequential Acoustic Solver",__DATE__,__TIME__)
...
!
      IF(controlVariables % stringValueForKey(solutionFileNameKey,LINE_LENGTH) /= "none")     THEN
         ...
         CALL sem % mesh % SaveSolution(sem % numberOfTimeSteps, timeIntegrator % time, solutionFileName, saveGradients, saveSource)
      END IF
