...
! !     ************************
! !     David: Clean IBM Mask
! !     ************************
!    if (self % IBM% active)en
...
! !     ************************
! !     Yingjie: Clean IBM Mask
! !     ************************
!    if (self % IBM% active) then
      
! !$omp parallel private(STLNum, eID, i, j, k)
!       do STLNum = 1, self% IBM% NumOfSTL
...
!                      if ( self % elements(eID)% STL(i,j,k) .eq. STLNum .and. &
!                           self % elements(eID)% isInsideBody(i,j,k) ) then
!                         self % elements(eID)% STL(i,j,k) = 0
!                         self % elements(eID)% isInsideBody(i,j,k) = .false.
!                      end if
!                   end do
!                end do
!             end do
...
! !          call e % ConstructIBM( e% Nxyz(1), e% Nxyz(2), e% Nxyz(3), self% IBM% NumOfSTL, self% IBM% wallfunction )
! !       end do
! ! !$omp end parallel do 

!       do STLNum = 1, self% IBM% NumOfSTL
!          call self% IBM% build( self % elements, self % faces, self % MPIfaces, &
!                                 self % NDOF, STLNum, self % child, .false., 0 )
!       end do

!    end if

! ************************
! Yingjie: Construct IBM Mask
! ************************
...
!$omp end parallel do 
      ...
      do STLNum = 1, self% IBM% NumOfSTL
         ...
         call OBB(STLNum)% ChangeObjsRefFrame( self% IBM% stl(STLNum)% ObjectsList, GLOBAL )
         
         call OBB(STLNum)% construct( self% IBM% stl(STLNum), self% IBM% plotOBB, self% IBM% AAB, .false. )
  
         ! call plotSTL( self% IBM% stl(STLNum), self% IBM% iter )

         call OBB(STLNum)% GetGLobalVertices()
  
         call OBB(STLNum)% ChangeObjsRefFrame( self% IBM% stl(STLNum)% ObjectsList, LOCAL )
  
         ! self% IBM% plot = .true.
         self% IBM% plotKDtree = .false.
         self% IBM% stl(STLNum)% show = .false.

         call self% IBM% constructSTL_KDtree( STLNum )
  
         if( self% IBM% ComputeBandRegion ) call self% IBM% DestroyBandRegion( STLNum )
         
         call self% IBM% build( self % elements, self % faces, self % MPIfaces, &
                                self % NDOF, STLNum, self % child, .false., self% IBM% iter )

         ! for surface monitor updated
         self% IBM% Integral(STLNum)% ListComputed = .false.
