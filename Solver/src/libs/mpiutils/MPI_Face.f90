...
module MPI_Face_Class
   ...
   type MPI_Face_t
      ...
      real(kind=RP), allocatable :: HfluxSend(:)
      real(kind=RP), allocatable :: HfluxRecv(:)
      contains
         ...
         procedure   :: SendHflux        => MPI_Face_SendHflux
         procedure   :: RecvHflux        => MPI_Face_RecvHflux
         ...
         procedure   :: WaitForHflux     => MPI_Face_WaitForHflux
   end type MPI_Face_t
...
!
         ...
         do domain = 1, MPI_Process % nProcs
            ...
            facesSet % faces(domain) % sizeHflux = NCONS * NDOFS(domain)
            ...
            if ( NDOFS(domain) .gt. 0 ) then
               ...
               safedeallocate(facesSet % faces(domain) % HfluxSend)
               safedeallocate(facesSet % faces(domain) % HfluxRecv)
               ...
               allocate( facesSet % faces(domain) % HfluxSend(NCONS * NDOFS(domain)) )
               allocate( facesSet % faces(domain) % HfluxRecv(NCONS * NDOFS(domain)) )
            end if
         end do
...
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
      subroutine MPI_Face_SendHflux(self, domain, nEqn)
         implicit none
         class(MPI_Face_t)      :: self
         integer, intent(in)    :: domain
         integer,    intent(in) :: nEqn
...
#ifdef _HAS_MPI_
         if ( self % no_of_faces .gt. 0 ) then
            call mpi_isend(self % HfluxSend, nEqn * self % nDOFs, MPI_DOUBLE, domain-1, DEFAULT_TAG, &
                           MPI_COMM_WORLD, dummyreq, ierr)
            call mpi_request_free(dummyreq, ierr)
         end if
#endif

      end subroutine MPI_Face_SendHflux

      subroutine MPI_Face_RecvHflux(self, domain, nEqn)
         implicit none
         class(MPI_Face_t)      :: self
         integer, intent(in)    :: domain
         integer,    intent(in) :: nEqn
...
#ifdef _HAS_MPI_
         if ( self % no_of_faces .gt. 0 ) then
            call mpi_irecv(self % HfluxRecv, nEqn * self % nDOFs, MPI_DOUBLE, domain-1, MPI_ANY_TAG, &
                           MPI_COMM_WORLD, self % HfluxRecv_req, ierr)
         end if
#endif

      end subroutine MPI_Face_RecvHflux
!
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
!     -------------------------------------------
...
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
      subroutine MPI_Face_WaitForHflux(self)
         implicit none
         class(MPI_Face_t)    :: self
#ifdef _HAS_MPI_
!
...
!        -----------------------------------
!
         call mpi_wait(self % HfluxRecv_req, status, ierr)
#endif

      end subroutine MPI_Face_WaitForHflux
!
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
...
!
      ...
      subroutine MPI_Face_Construct(self, no_of_faces)
         ...
         self % HfluxRecv_req = MPI_REQUEST_NULL
#endif
      ...
      subroutine MPI_Face_Destruct(self)
         ...
         safedeallocate(self % HfluxSend)
         safedeallocate(self % HfluxRecv)
#ifdef _HAS_MPI_
         self % Nrecv_req     = MPI_REQUEST_NULL
         self % Qrecv_req     = MPI_REQUEST_NULL
         self % gradQrecv_req = MPI_REQUEST_NULL
