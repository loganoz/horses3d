...
!
         ...
         DO id = 1, SIZE(elements) 
            ...
            DO k = 0, Nxyz(3)
               DO j = 0, Nxyz(2)
                  DO i = 0, Nxyz(1)
                     CALL self % dataSource % OutputVectorFromStateVector( outputVector, elements(id) % storage % Q(i,j,k,:) )
...
!
            ...
            DO k = 0, Nz
               DO j = 0, Ny
                  DO i = 0, Nx
                     CALL self % dataSource % outputVectorFromStateVector(array3DOld(:,i,j,k), elements(id) % storage % Q(i,j,k,:))
                  END DO  
               END DO   
            END DO
