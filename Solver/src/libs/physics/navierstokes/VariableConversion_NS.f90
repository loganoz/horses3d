...
!
      ...
      pure subroutine getDensityGradient_Entropy(Q,Q_x,Q_y,Q_z,rho_x,rho_y,rho_z)
         implicit none
         !-arguments---------------------------------------------------
         real(kind=RP), intent(in)  :: Q(NCONS)
         real(kind=RP), intent(in)  :: Q_x(NGRAD), Q_y(NGRAD), Q_z(NGRAD)
         ...
         rho_x = dot_product(Q, Q_x)
         rho_y = dot_product(Q, Q_y)
         rho_z = dot_product(Q, Q_z)

      end subroutine getDensityGradient_Entropy
!
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
!     -------------------------------------
!     Routines to get the pressure gradient
!     -------------------------------------
!
      ...
      pure subroutine getPressureGradient_Entropy(Q,Q_x,Q_y,Q_z,p_x,p_y,p_z)
         implicit none
         !-arguments---------------------------------------------------
         real(kind=RP), intent(in)  :: Q(NCONS)
         real(kind=RP), intent(in)  :: Q_x(NGRAD), Q_y(NGRAD), Q_z(NGRAD)
         ...
         p_x = p / Q(IRHO) * (dot_product(Q, Q_x) + p * Q_x(IRHOE))
         p_y = p / Q(IRHO) * (dot_product(Q, Q_y) + p * Q_y(IRHOE))
         p_z = p / Q(IRHO) * (dot_product(Q, Q_z) + p * Q_z(IRHOE))

      end subroutine getPressureGradient_Entropy
...
!     ---
      ...
      subroutine set_getGradients(grad_vars_)
         ...
         case(GRADVARS_ENTROPY)
            ...
            getDensityGradient   => getDensityGradient_ENTROPY
            getPressureGradient  => getPressureGradient_ENTROPY
