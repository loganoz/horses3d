...
!  ****************
!
   type, extends(GenericBC_t) ::  ZYDIYDirichletBC_t
#ifdef NAVIERSTOKES
      logical           :: isAdiabatic
      real(kind=RP)     :: Twall
      ...
      contains
         procedure         :: Destruct          => ZYDIYDirichletBC_Destruct
         procedure         :: Describe          => ZYDIYDirichletBC_Describe
#ifdef FLOW
         procedure         :: FlowState         => ZYDIYDirichletBC_FlowState
         procedure         :: FlowGradVars      => ZYDIYDirichletBC_FlowGradVars
         procedure         :: FlowNeumann       => ZYDIYDirichletBC_FlowNeumann
#endif

#ifdef CAHNHILLIARD
         procedure         :: PhaseFieldState   => ZYDIYDirichletBC_PhaseFieldState
         procedure         :: PhaseFieldNeumann => ZYDIYDirichletBC_PhaseFieldNeumann
         procedure         :: ChemPotState      => ZYDIYDirichletBC_ChemPotState
         procedure         :: ChemPotNeumann    => ZYDIYDirichletBC_ChemPotNeumann
#endif

#ifdef SCALAR
         procedure         :: ScalarState         => ZYDIYDirichletBC_ScalarState
         procedure         :: ScalarGradVars      => ZYDIYDirichletBC_ScalarGradVars
         procedure         :: ScalarNeumann       => ZYDIYDirichletBC_ScalarNeumann
#endif
#ifdef SCALAR_INS_V04
         procedure         :: ScalarState         => ZYDIYDirichletBC_ScalarState
         procedure         :: ScalarGradVars      => ZYDIYDirichletBC_ScalarGradVars
         procedure         :: ScalarNeumann       => ZYDIYDirichletBC_ScalarNeumann
#endif
   end type ZYDIYDirichletBC_t
!
!  *******************************************************************
!  Traditionally, constructors are exported with the name of the class
!  *******************************************************************
!
   interface ZYDIYDirichletBC_t
      module procedure ConstructZYDIYDirichletBC
   end interface ZYDIYDirichletBC_t
!
!  *******************
!  Function prototypes
!  *******************
...
!/////////////////////////////////////////////////////////
!
      function ConstructZYDIYDirichletBC(bname)
!
!        ********************************************************************
...
!
         implicit none
         type(ZYDIYDirichletBC_t)             :: ConstructZYDIYDirichletBC
...
!
         ...
         ! ConstructZYDIYDirichletBC % BCType = "freeslipwall"
         ConstructZYDIYDirichletBC % BCType = "zydiyDirichletBC"
         
         ConstructZYDIYDirichletBC % bname  = bname
         call toLower(ConstructZYDIYDirichletBC % bname)
...
!        -------------------------------------------------------------
         ...
         do 
            ...
            if ( index(trim(currentLine),"#define boundary") .ne. 0 ) then
               inside = CheckIfBoundaryNameIsContained(trim(currentLine), trim(ConstructZYDIYDirichletBC % bname)) 
            end if
!
...
!        -------------------------
         ...
         if ( trim(keyval) .eq. "adiabatic" ) then
            ConstructZYDIYDirichletBC % isAdiabatic = .true.
            ConstructZYDIYDirichletBC % ewall = 0.0_RP
            ConstructZYDIYDirichletBC % Twall = 0.0_RP
            ConstructZYDIYDirichletBC % invTwall = 0.0_RP
            ConstructZYDIYDirichletBC % wallType = 0.0_RP
         else
            ConstructZYDIYDirichletBC % isAdiabatic = .false.
            call GetValueWithDefault(bcdict, "wall temperature" , refValues % T, ConstructZYDIYDirichletBC % Twall     )
            ConstructZYDIYDirichletBC % ewall = ConstructZYDIYDirichletBC % Twall / (refValues % T*thermodynamics % gammaMinus1*dimensionless % gammaM2)
            ConstructZYDIYDirichletBC % invTwall = dimensionless % gammaM2*refValues % T / ConstructZYDIYDirichletBC % Twall
            ConstructZYDIYDirichletBC % wallType = 1.0_RP
         end if
#endif
#ifdef CAHNHILLIARD
         call GetValueWithDefault(bcdict, "contact angle", 90.0_RP, ConstructZYDIYDirichletBC % thetaw)
#endif
#ifdef INCNS_ADE
         if ( bcdict % ContainsKey("wall values") ) then
            ConstructZYDIYDirichletBC % vWall = getRealArrayFromString( bcdict % StringValueForKey("wall values",&
                                                                                           LINE_LENGTH))    
            
         else
            ConstructZYDIYDirichletBC % vWall = 0.0_RP
         end if
#endif
#ifdef SCALAR
         if ( bcdict % ContainsKey("wall values") ) then
      ...
      end function ConstructZYDIYDirichletBC

      subroutine ZYDIYDirichletBC_Describe(self)
!
!        ***************************************************
!              Describe the inflow boundary condition
         implicit none
         class(ZYDIYDirichletBC_t),  intent(in)  :: self
         write(STD_OUT,'(30X,A,A28,A)') "->", " Boundary condition type: ", "zydiyDirichletBC"
#ifdef NAVIERSTOKES
...
#ifdef INCNS_ADE
         write(STD_OUT,'(30X,A, A28,A,F10.2,A,F10.2,A,F10.2,A,F10.2,A,F10.2,A)')  &
                            "->", ' Wall velocity: ', "[",  &
                            self % vWall(1), ",", self % vWall(2), ",", self % vWall(3), ",", &
                            self % vWall(4), ",", self % vWall(5), "]"
#endif
         
      end subroutine ZYDIYDirichletBC_Describe
...
!/////////////////////////////////////////////////////////
!
      subroutine ZYDIYDirichletBC_Destruct(self)
         implicit none
         class(ZYDIYDirichletBC_t)    :: self

      end subroutine ZYDIYDirichletBC_Destruct
!
!////////////////////////////////////////////////////////////////////////////
!
...
!
#ifdef NAVIERSTOKES
      subroutine ZYDIYDirichletBC_FlowState(self, x, t, nHat, Q)
!
!        *************************************************************
...
!
      ...
      end subroutine ZYDIYDirichletBC_FlowState

      subroutine ZYDIYDirichletBC_FlowGradVars(self, x, t, nHat, Q, U, GetGradients)
!
!        *****************************************************************
...
!
      ...
      end subroutine ZYDIYDirichletBC_FlowGradVars

      subroutine ZYDIYDirichletBC_FlowNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
!
!        ***********************************************************
...
#endif
      end subroutine ZYDIYDirichletBC_FlowNeumann
#endif
!
!////////////////////////////////////////////////////////////////////////////
!
...
!        -----------------------------------------------
         ...
         ! Q(INSRHO:INSP) = -Q(INSRHO:INSP)
         ! Q(INSRHO:INSP) = 2*self % vWall-Q(INSRHO:INSP)
         Q(INSRHO:INSP) = self % vWall
...
!
      ...
      subroutine ZYDIYDirichletBC_PhaseFieldState(self, x, t, nHat, Q)
         implicit none
         class(ZYDIYDirichletBC_t),  intent(in)    :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
      ...
      end subroutine ZYDIYDirichletBC_PhaseFieldState

      subroutine ZYDIYDirichletBC_PhaseFieldNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
         implicit none
         class(ZYDIYDirichletBC_t),  intent(in) :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
...
!  
      ...
      end subroutine ZYDIYDirichletBC_PhaseFieldNeumann

      subroutine ZYDIYDirichletBC_ChemPotState(self, x, t, nHat, Q)
         implicit none
         class(ZYDIYDirichletBC_t),  intent(in)    :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
         real(kind=RP),       intent(in)    :: nHat(NDIM)
         real(kind=RP),       intent(inout) :: Q(NCOMP)
      end subroutine ZYDIYDirichletBC_ChemPotState

      subroutine ZYDIYDirichletBC_ChemPotNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
         implicit none
         class(ZYDIYDirichletBC_t),  intent(in) :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
      ...
      end subroutine ZYDIYDirichletBC_ChemPotNeumann
...
! *******************************************************************
#ifdef SCALAR
      subroutine ZYDIYDirichletBC_ScalarState(self, x, t, nHat, Q)
!
!        *************************************************************
!           Compute the state variables for a general wall
!
...
!        represents a solid wall.
!        -----------------------------------------------
         ! write(*,*) "self % vWall ==========", self % vWall

         Q(:) = self % vWall

      end subroutine ZYDIYDirichletBC_ScalarState

      subroutine ZYDIYDirichletBC_ScalarGradVars(self, x, t, nHat, Q, U, GetGradients)
!
!        **************************************************************
!           Use the interior velocity: Neumann BC!
!        **************************************************************
!
      ...
      end subroutine ZYDIYDirichletBC_ScalarGradVars

      subroutine ZYDIYDirichletBC_ScalarNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
!
!        ***************************************************************
!           Set homogeneous Neumann BCs everywhere
!        ***************************************************************
!        
      ...
      end subroutine ZYDIYDirichletBC_ScalarNeumann
#endif
! =================================================================================
! ========================  SCALAR Boundary condition  ============================
! =================================================================================
...
! =================================================================================
! ========================  SCALAR_INS_V04 Boundary condition  ============================
! =================================================================================
...
!        represents a solid wall.
!        -----------------------------------------------
         ! write(*,*) "self % vWall ==ZYDIr========", self % vWall

         Q(4:4) = self % vWall(4:4)
...
end module ZYDIYDirichletBCClass