#include "Includes.h"
module ZYDIYFreeSlipWallBCClass
   use SMConstants
   use PhysicsStorage
...
!  ****************
!
   type, extends(GenericBC_t) ::  ZYDIYFreeSlipWallBC_t
#ifdef NAVIERSTOKES
      logical           :: isAdiabatic
      real(kind=RP)     :: Twall
      ...
      contains
         procedure         :: Destruct          => ZYDIYFreeSlipWallBC_Destruct
         procedure         :: Describe          => ZYDIYFreeSlipWallBC_Describe
#ifdef FLOW
         procedure         :: FlowState         => ZYDIYFreeSlipWallBC_FlowState
         procedure         :: FlowGradVars      => ZYDIYFreeSlipWallBC_FlowGradVars
         procedure         :: FlowNeumann       => ZYDIYFreeSlipWallBC_FlowNeumann
#endif


#ifdef SCALAR_INS_V04
         procedure         :: ScalarState         => ZYDIYFreeSlipWallBC_FlowState
         procedure         :: ScalarGradVars      => ZYDIYFreeSlipWallBC_FlowGradVars
         procedure         :: ScalarNeumann       => ZYDIYFreeSlipWallBC_FlowNeumann
#endif

#ifdef CAHNHILLIARD
         procedure         :: PhaseFieldState   => ZYDIYFreeSlipWallBC_PhaseFieldState
         procedure         :: PhaseFieldNeumann => ZYDIYFreeSlipWallBC_PhaseFieldNeumann
         procedure         :: ChemPotState      => ZYDIYFreeSlipWallBC_ChemPotState
         procedure         :: ChemPotNeumann    => ZYDIYFreeSlipWallBC_ChemPotNeumann
#endif
   end type ZYDIYFreeSlipWallBC_t
!
!  *******************************************************************
!  Traditionally, constructors are exported with the name of the class
!  *******************************************************************
!
   interface ZYDIYFreeSlipWallBC_t
      module procedure ConstructZYDIYFreeSlipWallBC
   end interface ZYDIYFreeSlipWallBC_t
!
!  *******************
!  Function prototypes
!  *******************
...
!/////////////////////////////////////////////////////////
!
      function ConstructZYDIYFreeSlipWallBC(bname)
!
!        ********************************************************************
...
!
         implicit none
         type(ZYDIYFreeSlipWallBC_t)             :: ConstructZYDIYFreeSlipWallBC
...
!
         ...
         ! ConstructZYDIYFreeSlipWallBC % BCType = "freeslipwall"
         ConstructZYDIYFreeSlipWallBC % BCType = "zydiyfreeslipwall"
         
         ConstructZYDIYFreeSlipWallBC % bname  = bname
         call toLower(ConstructZYDIYFreeSlipWallBC % bname)
...
!        -------------------------------------------------------------
         ...
         do 
            ...
            if ( index(trim(currentLine),"#define boundary") .ne. 0 ) then
               inside = CheckIfBoundaryNameIsContained(trim(currentLine), trim(ConstructZYDIYFreeSlipWallBC % bname)) 
            end if
!
...
!        -------------------------
         ...
         if ( trim(keyval) .eq. "adiabatic" ) then
            ConstructZYDIYFreeSlipWallBC % isAdiabatic = .true.
            ConstructZYDIYFreeSlipWallBC % ewall = 0.0_RP
            ConstructZYDIYFreeSlipWallBC % Twall = 0.0_RP
            ConstructZYDIYFreeSlipWallBC % invTwall = 0.0_RP
            ConstructZYDIYFreeSlipWallBC % wallType = 0.0_RP
         else
            ConstructZYDIYFreeSlipWallBC % isAdiabatic = .false.
            call GetValueWithDefault(bcdict, "wall temperature" , refValues % T, ConstructZYDIYFreeSlipWallBC % Twall     )
            ConstructZYDIYFreeSlipWallBC % ewall = ConstructZYDIYFreeSlipWallBC % Twall / (refValues % T*thermodynamics % gammaMinus1*dimensionless % gammaM2)
            ConstructZYDIYFreeSlipWallBC % invTwall = dimensionless % gammaM2*refValues % T / ConstructZYDIYFreeSlipWallBC % Twall
            ConstructZYDIYFreeSlipWallBC % wallType = 1.0_RP
         end if
#endif
#ifdef CAHNHILLIARD
         call GetValueWithDefault(bcdict, "contact angle", 90.0_RP, ConstructZYDIYFreeSlipWallBC % thetaw)
#endif

#ifdef SCALAR_INS_V04
         if ( bcdict % ContainsKey("wall values") ) then
            ConstructZYDIYFreeSlipWallBC % vWall = getRealArrayFromString( bcdict % StringValueForKey("wall values",&
                                                                                           LINE_LENGTH))    
            
         else
            ConstructZYDIYFreeSlipWallBC % vWall = 0.0_RP
         end if
#endif
      ...
      end function ConstructZYDIYFreeSlipWallBC

      subroutine ZYDIYFreeSlipWallBC_Describe(self)
!
!        ***************************************************
!              Describe the inflow boundary condition
         implicit none
         class(ZYDIYFreeSlipWallBC_t),  intent(in)  :: self
         write(STD_OUT,'(30X,A,A28,A)') "->", " Boundary condition type: ", "ZYDIYFreeSlipWall"
#ifdef NAVIERSTOKES
      ...
      end subroutine ZYDIYFreeSlipWallBC_Describe
...
!/////////////////////////////////////////////////////////
!
      subroutine ZYDIYFreeSlipWallBC_Destruct(self)
         implicit none
         class(ZYDIYFreeSlipWallBC_t)    :: self

      end subroutine ZYDIYFreeSlipWallBC_Destruct
!
!////////////////////////////////////////////////////////////////////////////
!
...
!
#ifdef NAVIERSTOKES
      subroutine ZYDIYFreeSlipWallBC_FlowState(self, x, t, nHat, Q)
!
!        *************************************************************
...
!
      ...
      end subroutine ZYDIYFreeSlipWallBC_FlowState

      subroutine ZYDIYFreeSlipWallBC_FlowGradVars(self, x, t, nHat, Q, U, GetGradients)
!
!        *****************************************************************
...
!
      ...
      end subroutine ZYDIYFreeSlipWallBC_FlowGradVars

      subroutine ZYDIYFreeSlipWallBC_FlowNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
!
!        ***********************************************************
...
#endif
      end subroutine ZYDIYFreeSlipWallBC_FlowNeumann
#endif
!
!////////////////////////////////////////////////////////////////////////////
!
...
!
         ...
         vn = sum(Q(1:3)*nHat)
         ! write(*,*) " zy slip 0 ========== self % vWall =========0=", Q(1:N_INS)
         Q(1:N_INS) = Q(1:N_INS)
         ! Q(1:3) = Q(1:3) - 2.0_RP * vn * nHat
         ! write(*,*) " zy slip 0 ========== self % vWall =========1=", Q(1:N_INS)

         ! write(*,*) "self % vWall ==========", self % vWall

         Q(4) = self % vWall(4)
         Q(5:7) = self % vWall(5:7)
         ! write(*,*) " zy slip 0 ========== self % vWall =========2=", Q(5:7)
...
!
      ...
      subroutine ZYDIYFreeSlipWallBC_PhaseFieldState(self, x, t, nHat, Q)
         implicit none
         class(ZYDIYFreeSlipWallBC_t),  intent(in)    :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
      ...
      end subroutine ZYDIYFreeSlipWallBC_PhaseFieldState

      subroutine ZYDIYFreeSlipWallBC_PhaseFieldNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
         implicit none
         class(ZYDIYFreeSlipWallBC_t),  intent(in) :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
...
!  
      ...
      end subroutine ZYDIYFreeSlipWallBC_PhaseFieldNeumann

      subroutine ZYDIYFreeSlipWallBC_ChemPotState(self, x, t, nHat, Q)
         implicit none
         class(ZYDIYFreeSlipWallBC_t),  intent(in)    :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
         real(kind=RP),       intent(in)    :: nHat(NDIM)
         real(kind=RP),       intent(inout) :: Q(NCOMP)
      end subroutine ZYDIYFreeSlipWallBC_ChemPotState

      subroutine ZYDIYFreeSlipWallBC_ChemPotNeumann(self, x, t, nHat, Q, U_x, U_y, U_z, flux)
         implicit none
         class(ZYDIYFreeSlipWallBC_t),  intent(in) :: self
         real(kind=RP),       intent(in)    :: x(NDIM)
         real(kind=RP),       intent(in)    :: t
      ...
      end subroutine ZYDIYFreeSlipWallBC_ChemPotNeumann

#endif
end module ZYDIYFreeSlipWallBCClass