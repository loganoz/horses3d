...
#elif defined(SCALAR)
   !use RiemannSolvers_SLR             , only: RiemannSolver_dFdQ
   use VariableConversion              ,only: SLRGradientVariables 
#endif
#ifdef _HAS_MPI_
   use mpi
#endif
...
#endif
#if defined(SCALAR)
         call ViscousDiscretization % ComputeGradient (nEqn, nEqn, sem % mesh, time, SLRGradientVariables)
#ifdef _HAS_MPI_
!$omp single
         call sem % mesh % UpdateMPIFacesGradients(NGRAD)
!$omp end single
...
!        if (flowIsNavierStokes) then
 ...
 !        if (flowIsNavierStokes) then
   ...
   !#ifndef SPALARTALMARAS
    !     if (flowIsNavierStokes) 
         call ViscousDiscretization % RiemannSolver_Jacobians(f)
   !#endif
   ...
   !     ------------------------
         call slrViscousFlux(NCONS,NCONS,q,Q_xL,Q_yL,Q_zL,mu,beta,kappa,fv_3d)
   ...
   !     -----------------------------
         do dir=1, NDIM
            do i = 1, NCONS
               ...
               call slrViscousFlux(NCONS,NCONS,q,gradQL(:,1),gradQL(:,2),gradQL(:,3),mu,beta,kappa,fv_3d)
               newflux = fv_3d(:,IX)*nHat(IX) + fv_3d(:,IY)*nHat(IY) + fv_3d(:,IZ)*nHat(IZ)
               ! CALL BCs(f % zone) % bc % FlowNeumann( & 
               !                            x,       & 
               !                            time,    & 
   ...
   !     -----------------------
         do k12 = 0, e % Nxyz(3) ; do j12 = 0, e % Nxyz(2) ; do i2 = 0, e % Nxyz(1) ; do i1 = 0, e % Nxyz(1)
            ...
            do eq2 = 1, NCONS ; do eq1 = 1, NCONS 
               ...
               !call Matrix % AddToBlockEntry (e % GlobID, e % GlobID, j, i, MatEntries(eq1,eq2)/2.0) ! TODO: This can be improved by setting the whole matrix at once
   ...
   !     ------------------------
         do k12 = 0, e % Nxyz(3) ; do j2 = 0, e % Nxyz(2) ; do j1 = 0, e % Nxyz(2) ; do i12 = 0, e % Nxyz(1)
            ...
            do eq2 = 1, NCONS ; do eq1 = 1, NCONS 
               ...
               call Matrix % AddToBlockEntry (e % GlobID, e % GlobID, i, j, MatEntries(eq1,eq2))
               !call Matrix % AddToBlockEntry (e % GlobID, e % GlobID, j, i, MatEntries(eq1,eq2)/2.0)
   ...
   !           -----------------------------------------------------
               ...
               do eq2 = 1, NCONS ; do eq1 = 1, NCONS 
                  ...
                  call Matrix % AddToBlockEntry (e % GlobID, e % GlobID, i, j, MatEntries(eq1,eq2))
                  !call Matrix % AddToBlockEntry (e % GlobID, e % GlobID, j, i, MatEntries(eq1,eq2))
...
!                  call Matrix % AddToBlockEntry (e_plus % globID, e_minus % globID, j, i, MatEntries(eq1,eq2)/2.0)
