...
!
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
MODULE BlockJacobiSmootherClass
   use MatrixClass
   USE CSRMatrixClass
   USE SMConstants
   use DGSEMClass
...
!///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
!
   SUBROUTINE construct(this,DimPrb, globalDimPrb, nEqn,controlVariables,sem)
      IMPLICIT NONE
      !-----------------------------------------------------------
      CLASS(BJSmooth_t) , INTENT(INOUT), TARGET :: this
      INTEGER                  , INTENT(IN)            :: DimPrb
      INTEGER                  , INTENT(IN)            :: globalDimPrb
      integer                , intent(in)            :: nEqn
      TYPE(FTValueDictionary)  , INTENT(IN), OPTIONAL  :: controlVariables
      TYPE(DGSem), TARGET                  , OPTIONAL  :: sem
      ! procedure(MatrixShift_FCN), optional                       :: MatrixShiftFunc
      !-----------------------------------------------------------
...
!
   SUBROUTINE SetRHSValue(this, irow, value)
      IMPLICIT NONE
      !-----------------------------------------------------------
      CLASS(BJSmooth_t), INTENT(INOUT) :: this
...
!
   SUBROUTINE solve(this, nEqn, nGradEqn, ComputeTimeDerivative,tol,maxiter,time,dt, ComputeA)
      ...
      IMPLICIT NONE
      CLASS(BJSmooth_t), target, INTENT(INOUT) :: this
...
!     -----------------------------------------------------
      ...
      if ( present(ComputeA)) then
         if (ComputeA) then
            ! call this % Jacobian % Compute (this % p_sem, nEqn, time, this % A, ComputeTimeDerivative) ! FIX
...
!
   FUNCTION ComputeANextStep(this) RESULT(ComputeA)
      IMPLICIT NONE
      CLASS(BJSmooth_t), INTENT(IN) :: this
      LOGICAL                              :: ComputeA
...
!~      REAL(KIND=RP)                           :: buffer (size(x))
      ...
      ! shift = MatrixShift(dtsolve) ! FIXME
...
!~      this % p_sem % mesh % storage % Q = buffer
      ! Ax = ( this % p_F(this % Ur + x * eps, computeTimeDerivative) - this % F_Ur) / eps + shift * x ! FIXME
...
END MODULE BlockJacobiSmootherClass
