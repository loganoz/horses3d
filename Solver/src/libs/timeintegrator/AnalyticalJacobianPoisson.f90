...
!
        ...
        ! write(*,*) "spAxi%w     ===================  ", spAxi%w   
        ! write(*,*) "spAeta%w     ===================  ", spAeta%w   
        ! write(*,*) "spAzeta%w     ===================  ", spAzeta%w   
            ...
            do j12 = 0, e%Nxyz(2)
                do i2 = 0, e%Nxyz(1)
                    do i1 = 0, e%Nxyz(1)
                        ...
                        !                             * spAxi%w(index)       &
                        !                             ! * DOT_PRODUCT(                     &
                        !                             !     e%geom%jGradXi(:,i1,j12,k12)/ e%geom%Jacobian(i1, j12, k12),&
                        !                             !     e%geom%jGradXi(:,i1,j12,k12)/ e%geom%Jacobian(i1, j12, k12) &
                        !                             ! )
                        !                             * DOT_PRODUCT(                     &
                        ...
                        ! write (*,*) ")))e%geom%volume---(i1, j12, k12) ((#()))", e%geom%volume
                        ! write (*,*) ")))e%geom%Jacobian(i1, j12, k12) ((#()))", e%geom%Jacobian(i1, j12, k12)
                        ! write (*,*) ")))e%geom%jGradXi   (i1, j12, k12) ((#()))", e%geom%jGradXi  (:, i1, j12, k12)
                        ! write (*,*) ")))e%geom%jGradEta  (i1, j12, k12) ((#()))", e%geom%jGradEta (:, i1, j12, k12)
                        ! write (*,*) ")))e%geom%jGradZeta (i1, j12, k12) ((#()))", e%geom%jGradZeta(:, i1, j12, k12)
                        ...
                        write(*,*) "!     Xi contribution (dj*dk) ============= ", MatEntries
                        write(*,*) "! temp ============= ", j12,k12, i1, i2, temp
                        ! MatEntries = MatEntries  &
                        !                         * spAeta%w(j12) * spAzeta%w(k12) &
                        !                         * e%geom%Jacobian(i1, j12, k12)  
            ...
            do j2 = 0, e % Nxyz(2) 
                do j1 = 0, e % Nxyz(2) 
                    do i12 = 0, e % Nxyz(1)
                        ...
                        !                             * spAeta%w(index)                &
                        !                             ! * DOT_PRODUCT(                     &
                        !                             !         e%geom%jGradEta(:,i12,j1,k12)/e%geom%Jacobian(i12, j1, k12),&
                        !                             !         e%geom%jGradEta(:,i12,j1,k12)/e%geom%Jacobian(i12, j1, k12) &
                        !                             ! )                    
                        !                             * DOT_PRODUCT(                     &
                        !                                     e%geom%jGradEta(:,i12,index,k12)/e%geom%Jacobian(i12, index, k12),&
                        ...
                        ! MatEntries = MatEntries  &
                        !                         * spAxi%w(i12) * spAzeta%w(k12) &
                        !                         * e%geom%Jacobian(i12, j1, k12)  

                        MatEntries = MatEntries  &
                                                / spAEta%w(j1)

                        write(*,*) "e%geom%Jacobian(i12, j1, k12) ============== ",  e%geom%Jacobian(i12, j1, k12)  
                        write(*,*) "e%geom%Jacobian(i12, j2, k12) ============== ",  e%geom%Jacobian(i12, j2, k12)  


                        write(*,*) "!    Eta contribution (di*dk) ============= ", MatEntries
                    ...
                    do i12 = 0, e % Nxyz(1)
                        ...
                        do index = 0, e%Nxyz(3)
                            MatEntries = MatEntries &
                                                    ...
                                                    * spAzeta%w(index)            &
                                                    ! * DOT_PRODUCT(                     &
                                                    !         e%geom%jGradZeta(:,i12,j12,k1)/e%geom%Jacobian(i12,j12,k1),&
                                                    !         e%geom%jGradZeta(:,i12,j12,k1)/e%geom%Jacobian(i12,j12,k1) &
                                                    ! )             
                                                    * DOT_PRODUCT(                     &
                        ...
                        MatEntries = MatEntries  &
                                                * spAxi%w(i12) * spAeta%w(j12) &
                                                * e%geom%Jacobian(i12, j12, k1) 

                        write(*,*) "!    Zeta contribution (di*dj) ================== ", MatEntries
                ...
                do j1 = 0, e % Nxyz(2)  
                    do i2 = 0, e % Nxyz(1)  
                        do i1 = 0, e % Nxyz(1)  
                            ...
                            ! D(s2,j1)
                            ! ===================================
                            MatEntries = MatEntries + DOT_PRODUCT(           &
                                                        e%geom%jGradXi (:,i2,j1,k12)/ e %geom%Jacobian(i2, j1, k12),  &
                                                        e%geom%jGradEta(:,i1,j1,k12)/ e %geom%Jacobian(i1, j1, k12)   &
                                                    )                                                            &
                                                    *e %geom%Jacobian(i1, j1, k12)                                  &
                                                    * spAxi   %DT(i1,i2) * spAxi%w(i2)                             &
                                                    * spAeta  %DT(j2,j1) * spAeta%w(j1)                           &
                                                    * spAzeta% w(k12) 
                                                     
                            MatEntries = MatEntries + DOT_PRODUCT(             &
                                                        e%geom%jGradXi (:,i1,j1,k12)/ e %geom%Jacobian(i1, j1, k12),  &
                    ...
                    do i2 = 0, e % Nxyz(1)  
                        do i1 = 0, e % Nxyz(1)  
                            ...
                            MatEntries = MatEntries + DOT_PRODUCT(                                                   &
                                                    ...
                                                    * e %geom%Jacobian(i1, j12, k1)                                     &
                                                    * spAxi   %DT(i1,i2) * spAxi  %w(i2)                             &
                                                    * spAzeta %DT(k2,k1) * spAzeta%w(k1)                           &
                                                    * spAeta%w(j12) 
...
!     -------------------------
    ...
    !  -----------------------------------------------------------------------------------------------
    subroutine AnalyticalJacobianPoisson_OffDiagonalBlocks(mesh, nEqn, Matrix)
        implicit none
        !--------------------------------------------
        type(HexMesh), target    , intent(inout) :: mesh
        class(Matrix_t)          , intent(inout) :: Matrix
    ...
    !$omp end do
     ...
     subroutine Local_Get_BC_DiagonalBlock(f, neqn, e_plus, side, Matrix)
        ...
        real(kind=RP) :: MatEntries1(NCONS, NCONS)           ! Values of the matrix entries
        real(kind=RP) :: MatEntries2(NCONS, NCONS)           ! Values of the matrix entries
...
!
    ...
    if (e_plus%Nxyz(normAx_plus) /= 0 ) then
                    ...
                    ! write (*,*) "normLR ================, ", normLR
        ! write(*,*) "e_plus%geom%jGradXi,    ======BC V000===============, ", e_plus%geom%jGradXi
        ! write(*,*) "e_plus%geom%jGradEta,   ======BC V000===============, ", e_plus%geom%jGradEta
        ! write(*,*) "e_plus%geom%jGradZeta,  ======BC V000===============, ", e_plus%geom%jGradZeta
        ! write(*,*) "spAnorm_plus  %x                ======BC V000===============, ", spAnorm_plus  %x 
        write(*,*) "spAnorm_plus  %w                ======BC V000===============, ", spAnorm_plus  %w 
        write(*,*) "spAnorm_plus  %DT                ======BC V000===============, ", spAnorm_plus  %DT 
        write(*,*) "spAnorm_plus  %v                ======BC V000===============, ", spAnorm_plus  %v 
        write(*,*) "spAnorm_plus  %vd               ======BC V000===============, ", spAnorm_plus  %vd

        ! write(*,*) "normAx_plus, tanAx_plus ======================== ",normAx_plus, tanAx_plus
        ! write(*,*) "e_plus%Nxyz ======================== ",e_plus%Nxyz
                ...
                do i12=0,Nd1
                    do k2=0,Nd3
                        do k1=0,Nd3
                            ...
                            if(normAx_plus == 1) then
                                ! ---------------- normal direction of computational space is in the Xi dir 
                                Grad1=f%geom%GradEta  (:, i12,j12)
                                Grad2=f%geom%GradZeta (:, i12,j12)
                                Grad3=f%geom%GradXi   (:, i12,j12)
                            elseIF(normAx_plus == 2) then
                                ! ---------------- normal direction of computational space is in the Eta dir 
                                Grad1=f%geom%GradXi   (:, i12,j12)
                                Grad2=f%geom%GradZeta (:, i12,j12)
                                Grad3=f%geom%GradEta  (:, i12,j12)
                            else 
                                ! ---------------- normal direction of computational space is in the Zeta dir 
                                Grad1=f%geom%GradXi   (:, i12,j12)
                                Grad2=f%geom%GradEta  (:, i12,j12)
                                Grad3=f%geom%GradZeta (:, i12,j12)
                            endif
                

                            do index = 0, Nd3
                                MatEntries  = MatEntries  + spAtan1_plus%w ( i12)   &
                                                        *   spAtan2_plus%w ( j12)   &
                                                        *   spAnorm_plus%v ( k1, normAxSide_plus )  &
                                                        *   spAnorm_plus%v ( index, normAxSide_plus ) &
                                                        *   (   0                                                      &
                                                           +    spAnorm_plus%DT( k2,   index )  * Grad3(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAnorm_plus%DT( k2,   index )  * Grad3(2) *   f%geom%normal(2,i12,j12)   &              
                                                           +    spAnorm_plus%DT( k2,   index )  * Grad3(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            do index = 0, Nd1
                                MatEntries  = MatEntries  + spAtan1_plus%w ( i12)   &
                                                        ...
                                                        *   (   0                                                      &
                                                           +    spAtan1_plus%DT( i12,   index )  * Grad1(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAtan1_plus%DT( i12,   index )  * Grad1(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAtan1_plus%DT( i12,   index )  * Grad1(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            do index = 0, Nd2
                                MatEntries  = MatEntries  + spAtan1_plus%w ( i12)   &
                                                        ...
                                                        *   spAnorm_plus%v ( k2        , normAxSide_plus ) &
                                                        *   (   0                                                      &
                                                           +    spAtan2_plus%DT( j12,   index )  * Grad2(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAtan2_plus%DT( j12,   index )  * Grad2(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAtan2_plus%DT( j12,   index )  * Grad2(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 

                            end do
                            ! write(*,*) "new cartecion -=-=0-0-  ==============", MatEntries

                            ! -------------------------------------------------------------
                            MatEntries2 = MatEntries2 +   spAtan1_plus  %w( i12)  &
                                                        * spAtan2_plus  %w( j12)  &
                                                        * spAnorm_plus%v ( k1, normAxSide_plus ) &
                                                        * spAnorm_plus%v ( k2, normAxSide_plus ) &
                                                        * f%geom%jacobian(i12,j12)                                 
                            ...
                            write(*,*) " sigma term ============ ", spAnorm_plus%v ( k1, normAxSide_plus ) * spAnorm_plus%v ( k2, normAxSide_plus ) 
        
                            if(normAx_plus == 1) then
                                ! ---------------- normal direction of computational space is in the Xi dir 
                                baseRow = k1*NCONS + i12*EtaSpa1 + j12*ZetaSpa1
                                baseCol = k2*NCONS + i12*EtaSpa1 + j12*ZetaSpa1
                                jJacobian = e_plus%geom%jacobian(k2,i12,j12)
                                elseIF(normAx_plus == 2) then
                                ! ---------------- normal direction of computational space is in the Eta dir 
                                baseRow = i12*NCONS + k1*EtaSpa1 + j12*ZetaSpa1
                                baseCol = i12*NCONS + k2*EtaSpa1 + j12*ZetaSpa1
                                jJacobian = e_plus%geom%jacobian(i12,k2,j12)
                            else 
                                ! ---------------- normal direction of computational space is in the Zeta dir 
                                baseRow = i12*NCONS + j12*EtaSpa1 + k1*ZetaSpa1
                                baseCol = i12*NCONS + j12*EtaSpa1 + k2*ZetaSpa1
                                jJacobian = e_plus%geom%jacobian(i12,j12,k2)
                            endif
                            
                            MatEntries = 0&
                                        - 2.0_RP * 0.500_RP* MatEntries &
                                        + 2.0_RP * 0.500_RP* epsilon * MatEntries1  &
                                        + 2.0_RP * sigma * MatEntries2

                            MatEntries = MatEntries &
                                        /  spAtan1_plus %w(i12) / spAtan2_plus %w(j12) / spAnorm_plus%w(k1) / jJacobian   
...
!
        if (flowIsScalar) then
            ...
            ! write (*,*) "normLR ================, ", normLR
            ! write(*,*) "e_plus%geom%jGradXi,    ======off===============, ", e_plus%geom%jGradXi
            ! write(*,*) "e_plus%geom%jGradEta,   ======off===============, ", e_plus%geom%jGradEta
            ! write(*,*) "e_plus%geom%jGradZeta,  ======off===============, ", e_plus%geom%jGradZeta
            ! write(*,*) "spAnorm_plus  %x                ======off===============, ", spAnorm_plus  %x 
            ! write(*,*) "spAnorm_plus  %DT                ======off===============, ", spAnorm_plus  %DT 
            ! write(*,*) "spAnorm_plus  %v                ======off===============, ", spAnorm_plus  %v 
            ! write(*,*) "spAnorm_plus  %vd               ======off===============, ", spAnorm_plus  %vd
                ...
                do i12=0,Nd1
                    do k2=0,Nd3
                        do k1=0,Nd3
                            ...
                            ! write (*,*) ")))f%geom%jacobian(   ) ((#()))", f%geom%jacobian(i12,j12)  
                            ! write (*,*) ")))f%geom%GradEta  (:, i12,j12) ((#()))", f%geom%GradEta  (:, i12,j12) 
                            ! write (*,*) ")))f%geom%GradZeta (:, i12,j12) ((#()))", f%geom%GradZeta (:, i12,j12) 
                            ! write (*,*) ")))f%geom%GradXi   (:, i12,j12) ((#()))", f%geom%GradXi   (:, i12,j12) 

                            ! write (*,*) "f % rotation, side        =========== ", f % rotation, side         
                            ! write (*,*) "f % rotation, side        =========== ", f % rotation, side         
                            ! write (*,*) "f % rotation, other(side) =========== ", f % rotation, other(side)  
                            ! write (*,*) "f % rotation, other(side) =========== ", f % rotation, other(side)  
                            
                            
                            if (side == LEFT) then
                                MatEntries = 0&
                                        - 0.2500_RP* MatEntries           &
                                        + 0.2500_RP* epsilon * MatEntries1  &
                                        + 0.5_RP*sigma * MatEntries2
                ...
                do i12=0,Nd1
                    do k2=0,Nd3
                        do k1=0,Nd3
                            ...
                            do index = 0, Nd3
                                MatEntries  = MatEntries  + spAtan1_minus%w ( i12)   &
                                                        *   spAtan2_minus%w ( j12)   &
                                                        *   spAnorm_minus%v ( k1                      , normAxSide_minus )  &
                                                        *   spAnorm_minus%v ( index                   , normAxSide_minus ) &
                                                        *   (   0                                                      &
                                                           +    spAnorm_minus%DT( k2,   index )  * Grad3(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAnorm_minus%DT( k2,   index )  * Grad3(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAnorm_minus%DT( k2,   index )  * Grad3(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            do index = 0, Nd1
                                MatEntries  = MatEntries  + spAtan1_minus%w ( i12)   &
                                                        *   spAtan2_minus%w ( j12)   &
                                                        *   spAnorm_minus%v ( k1                      , normAxSide_minus )  &
                                                        *   spAnorm_minus%v ( k2                   , normAxSide_minus ) &
                                                        *   (   0                                                      &
                                                           +    spAtan1_minus%DT( i12,   index )  * Grad1(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAtan1_minus%DT( i12,   index )  * Grad1(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAtan1_minus%DT( i12,   index )  * Grad1(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            do index = 0, Nd2
                                MatEntries  = MatEntries  + spAtan1_minus%w ( i12)   &
                                                        ...
                                                        *   spAnorm_minus%v ( k2                   , normAxSide_minus ) &
                                                        *   (   0                                                      &
                                                           +    spAtan2_minus%DT( j12,   index )  * Grad2(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAtan2_minus%DT( j12,   index )  * Grad2(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAtan2_minus%DT( j12,   index )  * Grad2(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            ! -------------------------------------------------------------
                            MatEntries2 = MatEntries2 +   spAtan1_minus  %w( i12)  &
                                                        * spAtan2_minus  %w( j12)  &
                                                        * spAnorm_minus%v ( k1, normAxSide_minus ) &
                                                        * spAnorm_minus%v ( k2, normAxSide_minus ) &
                                                        * f%geom%jacobian(i12,j12)                                 
                ...
                do i12=0,Nd1
                    do k2=0,Nd3
                        do k1=0,Nd3
                            ...
                            ! write (*,*) "normAxSide_plus,normAxSide_minus =============",normAxSide_plus,normAxSide_minus 
                            ! write (*,*) "spAnorm_plus %v  =============",spAnorm_plus %v  
                            ! write (*,*) "spAnorm_minus %v =============",spAnorm_minus %v 
                            ! write (*,*) "spAnorm_minus %v =====normAxSide_plus ========",spAnorm_minus %v(:, normAxSide_plus  ) 
                            ! write (*,*) "spAnorm_minus %v =====normAxSide_minus========",spAnorm_minus%v (:, normAxSide_minus )
                            ! write (*,*) "spAnorm_plus %v ===== normAxSide_plus ========",spAnorm_plus%v (:, normAxSide_plus )
                            ! write (*,*) "spAnorm_plus %v ===== normAxSide_minus========",spAnorm_plus%v (:, normAxSide_minus )
                            ...
                            ! write(*,*) "new cartecion -=-=0-1-  ==============", MatEntries
                            do index = 0, Nd2
                                MatEntries  = MatEntries  + spAtan1_plus %w ( i12)   &
                                                        *   spAtan2_plus %w ( j12)   &
                                                        *   spAnorm_plus %v ( k1                      , normAxSide_plus  )  &
                            ...
                            ! write(*,*) "new cartecion -=-=0-2-  ==============", MatEntries
                        ...
                        do k1=0,Nd3
                            ...
                            do index = 0, Nd1
                                MatEntries  = MatEntries  + spAtan1_minus %w ( i12)   &
                                                        ...
                                                        *   spAnorm_plus  %v ( k2                   , normAxSide_plus   ) &
                                                        *   (   0                                                      &
                                                           +    spAnorm_plus%DT( i12,   index )  * Grad1(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAnorm_plus%DT( i12,   index )  * Grad1(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAnorm_plus%DT( i12,   index )  * Grad1(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
                            ...
                            do index = 0, Nd2
                                MatEntries  = MatEntries  + spAtan1_minus %w ( i12)   &
                                                        ...
                                                        *   spAnorm_plus  %v ( k2                   , normAxSide_plus   ) &
                                                        *   (   0                                                      &
                                                           +    spAnorm_plus%DT( j12,   index )  * Grad2(1) *   f%geom%normal(1,i12,j12)   &
                                                           +    spAnorm_plus%DT( j12,   index )  * Grad2(2) *   f%geom%normal(2,i12,j12)   &
                                                           +    spAnorm_plus%DT( j12,   index )  * Grad2(3) *   f%geom%normal(3,i12,j12)   &
                                                            )                                                                &
                                                        *   f%geom%jacobian(i12,j12)                                 
...
!
    ...
    call Matrix%SetBlockEntry(1, 1, 1, 1, 10000.0_RP)
