            ...
            do eID = 1, mesh % no_of_elements
               associate( Nx => mesh % elements(eID) % Nxyz(1), &
                 ...
                 do k = 0, Nz;  do j = 0, Ny;  do i = 0, Nx 
                   ...
                   q(1) = exp(x)*1.0_RP/10.0_RP + exp(y)*1.0_RP/10.0_RP + exp(z)*1.0_RP/10.0_RP + 1.0_RP
                   p    = q(1) 
                   q(2) = q(1)*COS(PI*(x+y+z)) 
                   q(3) = q(1)*SIN(PI*(x+y+z)) 
                   q(4) = q(1)*(0.5_RP*COS(PI*(x+y+z)) + 0.5_RP*SIN(PI*(x+y+z))) 
                   q(5) = p/(gamma - 1._RP) + q(1)*3.0_RP/2.0_RP
                   q(6) = q(1)*COS(PI*(x+y+z)) 
...
!
         ...
         subroutine UserDefinedState1(x, t, nHat, Q, thermodynamics_, dimensionless_, refValues_)
            ...
            type(Dimensionless_t),     intent(in)  :: dimensionless_
            type(RefValues_t),         intent(in)  :: refValues_
            real(kind=RP)      :: p,u,v,w

            u = 1.0_RP
            v = 1.0_RP
            w = 1.0_RP
#if defined(NAVIERSTOKES) & defined(SPALARTALMARAS)
            q(1) = exp(x(1))*1.0_RP/10.0_RP + exp(x(2))*1.0_RP/10.0_RP + exp(x(3))*1.0_RP/10.0_RP + 1.0_RP
            p    = q(1) 
            q(2) = q(1)*COS(PI*(x(1)+x(2)+x(3))) 
            q(3) = q(1)*SIN(PI*(x(1)+x(2)+x(3))) 
            q(4) = q(1)*(0.5_RP*COS(PI*(x(1)+x(2)+x(3))) + 0.5_RP*SIN(PI*(x(1)+x(2)+x(3)))) 
            q(5) = p/(thermodynamics_ % gamma - 1._RP) + q(1)*3.0_RP/2.0_RP
            q(6) = q(1)*COS(PI*(x(1)+x(2)+x(3))) 
#endif
...
!           ---------------
!
            CHARACTER(LEN=29)                  :: testName           = "MMS Spalart-Almaras"
