...
!
#if defined(NAVIERSTOKES)
            real(kind=RP), parameter :: residuals(5) = [ 1.7150195384091440E+01_RP, &
                                                         7.6439892166351541E+01_RP, &
                                                         4.5801210501160519E+01_RP, &
                                                         1.0180319252860737E+00_RP, &
                                                         1.0471633116043329E+03_RP]

            real(kind=RP), parameter           :: wake_u = 9.9999999995437383E-01_RP
            real(kind=RP), parameter           :: cd = 1.8107194259822396E+01_RP
            real(kind=RP), parameter           :: cl = -1.1778800157458136E-11_RP
            ...
            CALL FTAssertEqual(expectedValue = wake_u + 1.0_RP, &
                               ...
                               tol           = 1.d-11, &
                               msg           = "Wake final x-velocity at the point [2.0,0.0,0.5]")
            ...
            IF ( sharedManager % numberOfAssertionFailures() == 0 )     THEN
               ...
               WRITE(6,*) "This test case has no expected solution yet, only checks the residual after 75 iterations."
            ELSE
