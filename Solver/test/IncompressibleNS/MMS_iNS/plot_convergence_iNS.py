"""
plot_convergence_iNS.py
=======================
Reads errors.csv produced by run_convergence_iNS.py and generates
convergence plots for the HORSES3D Incompressible Navier-Stokes (iNS) MMS study.

USAGE
-----
    python3 plot_convergence_iNS.py

Or with a specific CSV file:
    python3 plot_convergence_iNS.py --csv path/to/errors.csv
...
def make_plots(data, p_data, t_final, outdir):
    ...
    fig.suptitle('Incompressible Navier-Stokes (iNS) — MMS Convergence',
...
def main():
    parser = argparse.ArgumentParser(
        description="Plot iNS MMS convergence from errors.csv")
    ...
    if not csv_path.exists():
        ...
        print("Run run_convergence_iNS.py first to generate results.")
        sys.exit(1)
