"""
run_convergence_iNS.py
=====================
Runs the HORSES3D Incompressible Navier-Stokes (iNS) MMS convergence study automatically.

USAGE
-----
1. Edit SECTION 1 — Paths (binary, meshes, results directory).
2. Edit SECTION 2 — Study parameters (mesh sizes, P values, dt, t_final).
3. Run:  python3 run_convergence_iNS.py

The script will:
  - Generate one control file per (mesh, P) combination
  - Run ./horses3d.ins for each case sequentially
...
Add --dry-run to print what would be run without executing anything:
  python3 run_convergence_iNS.py --dry-run
...
# Full path to the horses3d.ins binary
HORSES_BINARY = "../../../bin/horses3d.ins"
...
def main():
    parser = argparse.ArgumentParser(description="iNS MMS convergence runner")
    ...
    print(f"MMS Convergence Study — Incompressible Navier-Stokes (iNS)")
