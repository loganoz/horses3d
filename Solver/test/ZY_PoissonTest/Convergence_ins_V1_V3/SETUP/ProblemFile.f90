...
!
         ...
         real(kind=RP), intent(in)     :: t
         real(kind=RP), intent(in)     :: nHat(NDIM)
         real(kind=RP), intent(inout)  :: Q(N_INS)
         type(Thermodynamics_t),    intent(in)  :: thermodynamics_
         type(Dimensionless_t),     intent(in)  :: dimensionless_
         type(RefValues_t),         intent(in)  :: refValues_
...
!
            ...
            real(kind=RP) :: res1,res2,res3,res4,res5,res6,res7
            real(kind=RP) :: a, b, lambda
               ...
               do k = 0, Nz
                  do j = 0, Ny
                     do i = 0, Nx 
                        x = mesh % elements(eID) % geom % x(:,i,j,k)
                        ! gaussionWave = sin(x(IX)*PI*2)
                        ! gaussionWaveX = sin(x(IZ)*PI)
                        gaussionWaveX = sin(x(IZ) * PI) * cos(x(IX) * PI)
                        ...
                        ! res1 =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * 0)

                        ! res3 = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * 0)

                        ! res4 = 0.25_RP * (cos(2.0_RP * pi * x(IX)) + cos(2.0_RP * pi * x(IZ))) * exp(-4.0_RP * pi**2 * 0)


                        ! res5 =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * 0)

                        ! res7 = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * 0)


                        ! ! res1 = sin( x(IX) ) * (a *sin( a * x(IZ) ) - cos(a) * sinh( x(IZ) )  )
                        ! ! res3 = cos( x(IX) ) * (   cos( a * x(IZ) ) - cos(a) * sinh( x(IZ) )  )


                        ! mesh % elements(eID) % storage % q(1:7,i,j,k) = [res1,res2,res3,res4,res5,res6,res7]
                        ! mesh % elements(eID) % storage % q(3:3,i,j,k) = gaussionWaveX
                     end do
                  end do
               end do
               end associate
...
!
         ...
         !        -----------------------------------------------
            ...
            vn = sin((x(IX) - 0.4_RP*t)*PI)
            ! vn = 0
            write(*,*) "user defined vn, t ==========", vn, t
            write(*,*) "user defined x ==========", x
            ! write(*,*) "user defined vn ==========", vn, x, t
            ...
            Q(1) =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * t)
            Q(3) = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * t)

            Q(4) = 0.25_RP * (cos(2.0_RP * pi * x(IX)) + cos(2.0_RP * pi * x(IZ))) * exp(-4.0_RP * pi**2 * t)

            Q(5) =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * t)
            Q(7) = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * t)
...
!
            ...
            ! cos_ = -2*(a*a+b*b)*sin( a*x(1) -b*x(3)  ) * sin( b*x(1) + a*x(3)  ) + 0.0_RP
            ! cos_ = -2*(a*a+b*b)*sin( a*x(1) -b*x(3)  ) * sin( b*x(1) + a*x(3)  ) + 0.0_RP

            cos_ = -5.0 * a * a * sin(a * x(1) - 2.0 * b * x(3)) * sin(2.0 * a * x(3) + b * x(1)) &
                  - 6.0 * a * b * cos(a * x(1) - 2.0 * b * x(3)) * cos(2.0 * a * x(3) + b * x(1)) &
                  - 5.0 * b * b * sin(a * x(1) - 2.0 * b * x(3)) * sin(2.0 * a * x(3) + b * x(1))
            cos_ = cos_

            alpha = 0
            res4 = (0.0_RP + 2.0_RP * pi**2) * sin(pi * X(1)) * sin(pi * X(3))
            res4 = (0.0_RP) 
            ! res6 = (-1.0_RP + 2.0_RP * pi**2) * sin(pi * X(1)) * sin(pi * X(3))
            res5 = (1.0_RP + 2.0_RP * pi**2) * sin(pi * X(1)) * sin(pi * X(3)) + 2000_RP
            res6 = (1.0_RP + 2.0_RP * pi**2) * sin(pi * X(1)) * sin(pi * X(3)) + 5.0_RP
            res7 = (1.0_RP + 2.0_RP * pi**2) * sin(pi * X(1)) * sin(pi * X(3)) + 10.0_RP
            res5 = 0.0_RP
            res6 = 0.0_RP
            res7 = 0.0_RP
            ! res7 = (alpha + 2.0_RP * pi**2) * cos(pi * X(1)) * cos(pi * X(3))
...
!
            ...
            real(kind=RP)                      :: cos_, sin_
            real(kind=RP)                      :: res1,res2,res3,res4, res5
            real(kind=RP)                      :: res6, res7
            ...
            real(kind=RP) :: a,b
            real(kind=RP) :: dpdx, mu, U, h, pInlet, lenD
               ...
               do k = 0, Nz;  
                  do j = 0, Ny;  
                     do i = 0, Nx 
                        ...
                        cos_ = -sin( a*x(1) -b*x(3)  ) * sin( b*x(1) + a*x(3)  ) + 0.0_RP
               ...
               do k = 0, Nz;  
                  do j = 0, Ny;  
                     do i = 0, Nx 
                        ...
                        cos_ = -sin(a*x(1) - 2*b*x(3))*sin(2*a*x(3) + b*x(1))

                        ! res4= 3.0_RP/2.0_RP * x(1)**2 - 5.0_RP/2.0_RP * x(1) + 5.0_RP

                        res4 = 1_RP * sin(pi * X(1)) * sin(pi * X(3))
                  
                        res5 = 1_RP * sin(pi * X(1)) * sin(pi * X(3)) + 2000_RP
                        res6 = 1_RP * sin(pi * X(1)) * sin(pi * X(3)) + 5.0_RP
                        res7 = 1_RP * sin(pi * X(1)) * sin(pi * X(3)) + 10.0_RP
                        ...
                        res7 = 1.0_RP/2.0_RP/1.0_RP*(-1.0)
                        ! res7 = -1_RP * cos(pi * X(1)) * cos(pi * X(3))
                        ...
                        res4 = pInlet + dpdx*X(3)
                        
                        res7 = 1.0_RP / 2.0_RP / mu * dpdx * X(1)**2 &
                              + U / h * X(1) - 1.0_RP / 2.0_RP / mu * dpdx * h * X(1) 


            res1 =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * time)
            res3 = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * time)

            res4 = 0.25_RP * (cos(2.0_RP * pi * x(IX)) + cos(2.0_RP * pi * x(IZ))) * exp(-4.0_RP * pi**2 * time)

            res5 =  10.0_RP * cos(pi * x(IX)) * sin(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * time)
            res7 = -10.0_RP * sin(pi * x(iX)) * cos(pi * x(IZ)) * exp(-2.0_RP * 1.0_RP * pi**2 * time)

            res2 = 1.0_RP
            res6 = 1.0_RP
                        Q = mesh % elements(eID) % storage % q(:,i,j,k) 
                        
                        locErr = (Q-[res1,res2,res3, res4, res5,res6,res7])**2
                        ...
                        L2Err = L2Err + wi*wj*wk*Jijk*locErr
                        L2ErrRelative = L2ErrRelative +  wi*wj*wk*Jijk* [cos_*cos_,cos_*cos_, &
                                                                           cos_*cos_, res4*res4, & 
                                                                           res5*res5,res6*res6, & 
                                                                           res7*res7 & 
                                                                           ]
