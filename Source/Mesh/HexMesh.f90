...
!
      SUBROUTINE SetBoundaryTypes( this ) 
         ...
         DO j = 1, this%numberOfFaces 
            IF( this%edges(j)%elementIDs(2) == HMESH_NONE )     THEN
!
!              ---------------------------------
!              Set the edge boundary information
!              ---------------------------------
!
               this%edges(j)%edgeType = HMESH_BOUNDARY
...
!
               DO k = 1, 2 
                  this%nodes(this%edges(j)%nodeIds(k))%nodeType = HMESH_BOUNDARY
               ...
               END DO
            ELSE
               this%edges(j)%edgeType = HMESH_INTERIOR
            END IF
         END DO
